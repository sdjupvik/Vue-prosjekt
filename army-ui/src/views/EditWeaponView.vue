...
let weaponId = Number(router.currentRoute.value.params["id"]);

onMounted(async () => {
  let weapon = await apiGet(`weapon/${weaponId}`)
  newWeapon.value.name = weapon.name
  newWeapon.value.type = weapon.weaponType.id
  newWeapon.value.description = weapon.description
  newWeapon.value.isNuclear = weapon.isNuclear
  newWeapon.value.mass = weapon.mass
  newWeapon.value.length = weapon.length
  newWeapon.value.range = weapon.range
})

async function updateWeapon() {
  await apiPut(`weapon/${weaponId}`, newWeapon.value)
  router.push('/weapons')
}
</script>

<template>
  <form class="text-army-500 custom-class" @submit.prevent="updateWeapon">
